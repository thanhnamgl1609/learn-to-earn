...
import { NftMeta, PinataRes } from '@_types/nftIdentity';
...
const pinataDomain = process.env.NEXT_PUBLIC_PINATA_DOMAIN as string;
...
const NftCreate: NextPage = () => {
  ...
  const getSignedData = async () => {
    ...
    const signedData = await ethereum?.request({
      ...
      params: [JSON.stringify(messageToSign), account.data, messageToSign.id],
    });
  ...
  const handleImage = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setNftMeta({
        ...nftMeta,
        image: `${pinataDomain}/ipfs/${data.IpfsHash}`,
      });
    } catch (e) {
      console.error(e);
    }
  ...
  const handleAttributeChange = (e: ChangeEvent<HTMLInputElement>) => {
    const { name, value } = e.target;
    const { attributes } = nftMeta;
    const changedAttribute = attributes.find(
      ({ trait_type: attribute }) => attribute === name
    );
    if (changedAttribute) {
  ...
  const uploadMetadata = async () => {
    try {
      ...
      const data = res.data as PinataRes;
      setNftURI(`${pinataDomain}/ipfs/${data.IpfsHash}`);
    } catch (e: any) {
